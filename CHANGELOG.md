...
- Make QMD backend integration legacy-only according to the generated OpenClaw schema. Before upgrading to a retired schema such as 2026.9.3, remove `memory.backend`, `memory.qmd`, and `memory.search.qmd` from Nix-authored config; no automatic migration is performed. Preserve legacy opt-in, standalone QMD CLI packaging, and explicit model prewarming, with checks for both schema contracts (2026-09-09).
